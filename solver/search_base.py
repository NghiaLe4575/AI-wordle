...
class OptimizedGraphSearchSolver:
    ...
    def solve(
        ...
        shared_feedback_table: FeedbackTable = None,
    ) -> SolverResult:
        ...
        if (OptimizedGraphSearchSolver._shared_feedback_table is None or 
        OptimizedGraphSearchSolver._shared_word_list_len != len(word_pool)):
            OptimizedGraphSearchSolver._shared_word_list = list(word_pool)
            OptimizedGraphSearchSolver._shared_word_list_len = len(word_pool)
            OptimizedGraphSearchSolver._shared_word_list = list(word_pool)
            OptimizedGraphSearchSolver._shared_feedback_table = shared_feedback_table
    ...
    def _filter_candidates_fast(self, possible_indices: set[int], guess_idx: int, 
                                ...
                                feedback_table: FeedbackTable) -> set[int]:
        # If feedback_table has adjacency lookups, use them
        # Otherwise, precompute feedback for all (guess, word) pairs at initialization
        
        
        # Option B: Cache feedback results per (guess_idx, feedback) pair
        result: set[int] = set()
        for idx in possible_indices:
            if feedback_table.get_feedback_idx(guess_idx, idx) == feedback:
                result.add(idx)
        return result
