...
Optimized Wordle solver using BFS, DFS, UCS, and A* with information-theoretic cost functions. 
...
wordle-solver/
├── data/
│   └── words_max.txt                 # 5,717 valid 5-letter words
├── engine/
│   └── engine.py                 # Game logic: feedback, state tracking
├── solver/
│   ├── letter_feedback.py               # Mark enum, evaluate_guess()
│   ├── feedback_matrix.py         # Cached feedback lookups (~400K entries)
│   ├── search_state.py                  # CompactState: hashable search state
...
## Quick Start
For the application to run smoothly without having to wait for the app to build a feedback table. Download the fbmat_14855_895e583f.pkl feedback table at the google drive link:
